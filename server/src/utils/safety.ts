...
export function checkInvalidAttempts(
  ...
  if (newCount >= SAFETY_LIMITS.MAX_INVALID_ATTEMPTS) {
    return {
      isValid: false,
      reason:
        "I'm having trouble understanding your responses. Let's start fresh - please click one of the quick action buttons below.",
      shouldTerminate: true,
...
export async function validateUserInput(
  ...
  const hasRelevantKeyword = lostFoundKeywords.some((keyword) => messageLower.includes(keyword));
