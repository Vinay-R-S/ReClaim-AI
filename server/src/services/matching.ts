...
async function calculateImageScore(
  ...
  try {
    ...
    log.error('Image comparison failed:', error);
    return 0;
  }
}
      ...
      [
        {
          role: 'system',
          content:
            'You are a semantic matching engine for lost and found items. Output only a number.',
        },
 ...
 */
export async function findMatchesForLostItem(lostItem: {
  name: string;
  description: string;
  tags?: string[];
  location?: string;
 ...
 */
export async function findMatchesForFoundItem(foundItem: {
  name: string;
  description: string;
  tags?: string[];
  coordinates?: Coordinates;
