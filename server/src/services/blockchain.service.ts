...
async function initializeBlockchain(forceReconnect = false) {
  ...
  const privateKey = env.blockchain.adminPrivateKey;
  const contractAddress = env.blockchain.contractAddress;
  ...
  for (const rpcUrl of SEPOLIA_RPC_URLS) {
    try {
      log.info(`Trying RPC: ${rpcUrl.substring(0, 40)}...`);
      ...
      log.info('Admin wallet:', wallet.address);
      log.info('Contract:', contractAddress);
      log.info('RPC:', rpcUrl);

      return; // Success - no need to try other URLs!
    } catch (error: any) {
      log.warn(`RPC ${rpcUrl.substring(0, 30)}... failed:`, error.message);
      lastError = error;
      // Try next RPC
      provider = null;
      wallet = null;
  ...
  throw new Error(
    `Blockchain initialization failed: ${lastError?.message || 'All RPC endpoints failed'}`,
  );
}
...
export async function recordHandoverOnBlockchain(data: {
  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      if (needsReconnect) {
        log.info(`Retry attempt ${attempt + 1}/${MAX_RETRIES} - reconnecting to different RPC...`);
        provider = null;
        wallet = null;
        contract = null;
        isBlockchainInitialized = false;
      ...
      log.info(`Recording handover ${data.matchId} on blockchain...`);
      ...
      log.info(
        `✓ Handover recorded successfully! Block: ${receipt.blockNumber}, Gas used: ${receipt.gasUsed.toString()}`,
      );
      log.info(`Transaction complete - blockchain is ready for next operation`);
      ...
      if (shouldReconnect) {
        log.warn(`Network error (attempt ${attempt}/${MAX_RETRIES}):`, error.message);
        needsReconnect = true; // Mark that we need to try a different RPC
      } else {
        log.error(`Blockchain error (attempt ${attempt}/${MAX_RETRIES}):`, error.message);
 ...
 */
export async function verifyHandoverOnBlockchain(matchId: string): Promise<boolean> {
  try {
    ...
    log.error('Blockchain verification failed:', error);
    return false;
  }
}
...
export async function getHandoverFromBlockchain(matchId: string) {
  try {
    ...
    log.error('Failed to get handover from blockchain:', error);
    return null;
  }
}
...
export async function getTotalHandovers(): Promise<number> {
  try {
    ...
    log.error('Failed to get total handovers:', error);
    return 0;
  }
}
...
export async function getAdminBalance(): Promise<string> {
  try {
    ...
    log.error('Failed to get admin balance:', error);
    return '0';
  }
}
