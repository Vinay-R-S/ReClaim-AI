...
export function validateHandoverCriteria(lostItem: Item, foundItem: Item): string | null {
  ...
  if (lostItem.coordinates && foundItem.coordinates) {
    ...
    if (
      ...
      (!lostItem.coordinates || !foundItem.coordinates)
    ) {
...
export async function initiateHandover(
  ...
  try {
    ...
    if (validationWarning) {
      log.info(`[HANDOVER] Validation warning (proceeding anyway): ${validationWarning}`);
    }
...
export async function verifyHandoverCode(
  ...
  try {
    // 1. Get Code Record
    const snapshot = await collections.handoverCodes.where('matchId', '==', matchId).limit(1).get();
...
async function completeHandover(matchId: string, codeDocId: string, data: HandoverCode) {
  ...
  try {
    const blockchainEnabled = env.blockchain.enabled;
    ...
    if (blockchainEnabled) {
      ...
      if (result.success) {
        ...
        log.info(`View on Etherscan: https://sepolia.etherscan.io/tx/${result.txHash}`);
    ...
    log.error('Blockchain integration error:', blockchainError.message);
  }
}
 ...
 */
async function blockUserAndReset(
  matchId: string,
  codeDocRef: any,
  lostItemId: string,
  foundItemId: string,
  ...
  if (lostItemDoc.exists) {
    ...
    if (userId) {
      ...
      if (userRole !== 'admin') {
        ...
        log.info(`User ${userId} blocked for failed verification attempts`);
      } else {
        log.info(`Skipping block for admin user ${userId}`);
      }
    }
  }
