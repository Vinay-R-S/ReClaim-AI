...
export async function updateCredits(
  ...
  try {
    ...
    log.info(
      `Credits updated for user ${userId}: ${amount > 0 ? '+' : ''}${amount}, new balance: ${newBalance}`,
    );
...
export async function getCreditHistory(
  ...
  try {
    ...
    log.error('Error getting credit history:', error);
    return [];
  }
}
...
export async function initializeCredits(userId: string): Promise<void> {
  try {
    ...
    log.error('Error initializing credits:', error);
  }
}
